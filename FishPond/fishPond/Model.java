 ...
 *                                                                           
 * @author Fawzi Emad, Thompson Nguyen            
 */
public class Model {
	...
	public boolean isSpaceAvailable(int r, int c) {
		...
		for(Plant planty:plants) {
			if(planty.getRow() == r && planty.getCol() == c) {
				isPlant = true;
				break;
			}
		}
		if(isFish || isPlant || landscape[r][c]==ROCK) {
			return false;
		} else {
			return true;
		}
	...
	public void removeDeadFish() {
		for(Fish fishy: new ArrayList<Fish>(fish)) {
			if(!fishy.isAlive()) {
				fish.remove(fishy);
			}
		}
	...
	private boolean fishIsSurroundedByRocks(Fish f) {
		...
		if(landscape[row][col-1]==ROCK &&
				landscape[row][col+1]==ROCK &&
				landscape[row-1][col]==ROCK &&
				landscape[row+1][col]==ROCK) {
			return true;
		} else {
			return false;
	...
	public void turnFish() {
		for(Fish fishy: fish) {
			if(fishIsSurroundedByRocks(fishy) == true) {
				continue;
			}
			if(fishy.getDirection() != Fish.UP &&
					fishy.getDirection() != Fish.DOWN &&
					fishy.getDirection() != Fish.LEFT &&
					fishy.getDirection() != Fish.RIGHT) {
				throw new IllegalFishDirectionException(fishy.getDirection());
			}
			while(fishy.getDirection() == Fish.UP &&
					landscape[fishy.getRow()-1][fishy.getCol()] ||
					fishy.getDirection() == Fish.DOWN &&
					landscape[fishy.getRow()+1][fishy.getCol()] ||
					fishy.getDirection() == Fish.LEFT &&
					landscape[fishy.getRow()][fishy.getCol()-1] ||
					fishy.getDirection() == Fish.RIGHT &&
					landscape[fishy.getRow()][fishy.getCol()+1]) {
	...
	public void moveFish() {
		for(Fish fishy: fish) {
			if(fishy.isAlive()) {
				...
				for(Plant plant: plants) {
					if(plant.isAlive() &&
							plant.getCol()==fishy.getCol() &&
							plant.getRow()==fishy.getRow()) {
						fishEatPlant(fishy, plant);
					}
				}
				for(int i=0; i<fish.size(); i++) {
					if(fish.indexOf(fishy)!=i &&
							fish.get(i).getRow()==fishy.getRow() &&
							fish.get(i).getCol()==fishy.getCol()) {
						fishy.fight(fish.get(i));
					}
				}
			}
		}
	...
	public void addPlant(Plant p) {
		...
		for(Plant plant: plants) {
			if(plant.getCol()==p.getCol() &&
					plant.getRow()==p.getRow()) {
				throw new IllegalPlantPositionException
	...
	public void addFish(Fish f){
		...
		for(Fish fishy: fish) {
			if(fishy.getCol()==f.getCol() &&
					fishy.getRow()==f.getRow()) {
				throw new IllegalFishPositionException
